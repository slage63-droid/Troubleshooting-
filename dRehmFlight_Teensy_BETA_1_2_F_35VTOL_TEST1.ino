...
*/
// Перехід з Oneshot125 на PWM
//9.02.2026 перехід з TEST на TEST1. проведено зміни by GitHub Copilot.Моніторинг моторів реагує на арм -і видає 1000 на 1500
//
//
//
//
...
void setup() {
  ...
  m1_command_PWM = 1000;  // PWM мінімум для arm
  m2_command_PWM = 1000;
  m3_command_PWM = 1000;
  m4_command_PWM = 1000;
  m5_command_PWM = 1000;
  ...
  commandMotors();
  delay(50);
  commandMotors(); // Більше імпульсів для надійного arm
...
void scaleCommands() {
  ...
  /*
   * mX_command_scaled variables from the mixer function are scaled to 1000-2000us for Standard PWM protocol.
   * HOBBYWING XROTOR 60A BLHeli_32 ESCs require standard PWM (NOT OneShot125).
   * sX_command_scaled variables are scaled to 0-180 for the servo library.
   */
  //Scaled to 1000us - 2000us for standard PWM protocol
  m1_command_PWM = m1_command_scaled*500 + 1500;
  m2_command_PWM = m2_command_scaled*500 + 1500;
  m3_command_PWM = m3_command_scaled*500 + 1500;
  m4_command_PWM = m4_command_scaled*500 + 1500;
  m5_command_PWM = m5_command_scaled*500 + 1500;
  m6_command_PWM = m6_command_scaled*500 + 1500;
...
void commandMotors() {
  //DESCRIPTION: Send pulses to motor pins, standard PWM protocol
  /*
   * Sends standard PWM pulses (1000-2000us) to ESC control pins.
   * Compatible with HOBBYWING XROTOR 60A BLHeli_32 ESCs.
   * The pulse length being sent is mX_command_PWM, computed in scaleCommands().
   */
  int wentLow = 0;
  ...
  while (wentLow < 6) { //keep going until final (6th) pulse is finished
    timer = micros();
...
void throttleCut() {
  ...
  /*
   * Monitors the state of radio command channel_5_pwm and directly sets the mx_command_PWM values to minimum (1000 is
   * minimum for standard PWM protocol) if channel 5 is high. This is the last function 
   * called before commandMotors() is called so that the last thing checked is if the user is giving permission to command
   * the motors to anything other than minimum value. Safety first. 
   */
  if (channel_5_pwm > 1500) {
    m1_command_PWM = 1000;  // PWM мінімум для standard protocol
    m2_command_PWM = 1000;
    m3_command_PWM = 1000;
    m4_command_PWM = 1000;
    m5_command_PWM = 1000;
...
void debugMotorPins() {
  ...
  if (current_time - lastPrint > 100000) {  // Кожні 100ms
    lastPrint = current_time;
    
    Serial.print("Pin states - ");
    Serial.print("m1Pin(0)="); Serial.print(digitalRead(m1Pin));
    Serial.print(" m2Pin(1)="); Serial.print(digitalRead(m2Pin));
    Serial.print(" m3Pin(2)="); Serial.print(digitalRead(m3Pin));
    Serial.print(" | PWM: m1="); Serial.print(m1_command_PWM);
    Serial.print(" m2="); Serial.print(m2_command_PWM);
    Serial.print(" m3="); Serial.println(m3_command_PWM);
  }
}
