...
//THIS VERSION FOR THE F-35 FEATURES ANGLE MODE FOR HOVER AND IS NOT EXTENSIVELY TESTED. RATE MODE IN HOVER HIGHLY REOMMENDED FOR MAXIMUM CONTROL AUTHORITY.  
...
//                                                Результати тестів                                                       //
//========================================================================================================================//
// код оснований на версії   dRehmFlight_Teensy_1_2_F_35VTOL_AngleHover_test.ino
// Радіо команди проходять,гіроском ,сервокоманди проходять
// ВИ НАМАГАЄТЕСЯ КЕРУВАТИ ESC ЧЕРЕЗ OneShot125, А ЦЕЙ ESC НЕ ПІДТРИМУЄ OneShot125 line 353 - ESC не працює
// коректувати положення серво в коді - лінії  1053-1069
// 
// тест рядок 407
// Servo trims - tune accordingly рядок 1052
// Flight mode 1 - Hover  line 1072
// Flight mode 2 - Transition line 1104
// Flight mode 3 - Forward Flight line1142
// додано код лінії  -39-42       компіляція ок
// додано код лінії  -319-321 - компіляція -ok
// додано код лінії  - компіляція -
// додано код лінії  - компіляція -
// додано код лінії  - компіляція -
//
//
...
float maxRoll = 160.0;     //Max roll angle in degrees for angle mode (maximum 60 degrees), deg/sec for rate mode 
float maxPitch = 160.0;    //Max pitch angle in degrees for angle mode (maximum 60 degrees), deg/sec for rate mode
...
float Ki_roll_rate = 0.3;     //Roll I-gain - rate mode
float Kd_roll_rate = 0.0002;  //Roll D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)
float Kp_pitch_rate = 0.2;   //Pitch P-gain - rate mode
float Ki_pitch_rate = 0.3;    //Pitch I-gain - rate mode
...
//DSHOT(OneShot125) ESC pin outputs:
const int m1Pin = 0;
const int m2Pin = 1;
const int m3Pin = 2;
const int m4Pin = 3;
...
void setup() {
  ...
  //DSHOT ARM          //Arm OneShot125 motors
  esc1.setThrottle(0);//m1_command_PWM = 125; //command OneShot125 ESC from 125 to 250us pulse length
  esc2.setThrottle(0);//m2_command_PWM = 125;
  esc3.setThrottle(0);//m3_command_PWM = 125;
  //m4_command_PWM = 125;
  //m5_command_PWM = 125;
  // m6_command_PWM = 125;
  //commandMotors();
...
void loop() {
  ...
  controlRATE(); //stabilize on rate setpoint -test- was comented

  if (channel_6_pwm > 1700) { //hover
    maxRoll = 30.0;    //max roll angle in degrees for angle mode, deg/sec for rate mode
    maxPitch = 30.0;   //max pitch angle in degrees for angle mode, deg/sec for rate mode
    maxYaw = 160.0;    //max yaw rate in degrees/sec

    controlANGLE();
  }
  if (channel_6_pwm < 1700) { //Transition and forward flight
    controlRATE();
  }
  ...
  //Command actuators
  //sends command pulses to each motor pin using OneShot125 protocol
...
void controlMixer() {
  ...
  float right_ail_trim_hover = 0.0; //adjust in 0.1 incriments to bring right aileron to ~70 degrees down when in hover mode
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if (channel_6_pwm > 1700) {
    ...
    m2_command_scaled = 0.7*thro_des + pitch_PID - roll_PID; //right propeller
    m3_command_scaled = 0.7*thro_des + pitch_PID + roll_PID; //left propeller
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if (channel_6_pwm < 1300) {
    maxRoll = 440.0;    //max roll angle in degrees for angle mode, deg/sec for rate mode
    maxPitch = 440.0;   //max pitch angle in degrees for angle mode, deg/sec for rate mode
    maxYaw = 200.0;    //max yaw rate in degrees/sec
    ...
    Kp_roll_rate = 0.11;     //Roll P-gain - rate mode
    Ki_roll_rate = 0.3;     //Roll I-gain - rate mode
    Kd_roll_rate = 0.0004;  //Roll D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)
    Kp_pitch_rate = 0.17;    //Pitch P-gain - rate mode
    Ki_pitch_rate = 0.3;    //Pitch I-gain - rate mode
    Kd_pitch_rate = 0.0004; //Pitch D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)
  
    Kp_yaw = 0.35;           //Yaw P-gain
    Ki_yaw = 0.05;          //Yaw I-gain
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  //Unused actuators - set to zero regardless of flight mode
  m4_command_scaled = 0;
  m5_command_scaled = 0;
  m6_command_scaled = 0;
  s6_command_scaled = 0;
...
void scaleCommands() {
  ...
  // ---- DSHOT throttle scaling ----
  m1_command_PWM = constrain(m1_command_scaled * 2000, 0, 2000);
  m2_command_PWM = constrain(m2_command_scaled * 2000, 0, 2000);
  m3_command_PWM = constrain(m3_command_scaled * 2000, 0, 2000);
  m4_command_PWM = constrain(m4_command_scaled * 2000, 0, 2000);
  m5_command_PWM = constrain(m5_command_scaled * 2000, 0, 2000);
  m6_command_PWM = constrain(m6_command_scaled * 2000, 0, 2000);

  // ---- Серво частина НЕ змінюється ----
  s1_command_PWM = constrain(s1_command_scaled*180, 0, 180);
  s2_command_PWM = constrain(s2_command_scaled*180, 0, 180);
  s3_command_PWM = constrain(s3_command_scaled*180, 0, 180);
  s4_command_PWM = constrain(s4_command_scaled*180, 0, 180);
  s5_command_PWM = constrain(s5_command_scaled*180, 0, 180);
  s6_command_PWM = constrain(s6_command_scaled*180, 0, 180);
  s7_command_PWM = constrain(s7_command_scaled*180, 0, 180);
...
void throttleCut() {
  if (channel_5_pwm > 1500) {
    ...
    esc2.setThrottle(0);
    esc3.setThrottle(0);
    //якщо будуть інші ESC — додаси їх також
  }
}
